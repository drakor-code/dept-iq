...
export default function SupplierDetailsPage() {
  ...
  const [supplier, setSupplier] = useState<SupplierWithTransactions | null>(null)
  ...
  // بيانات إضافة دين
  const [debtData, setDebtData] = useState<Omit<CreateSupplierTransactionData, 'supplierId' | 'type'>>({
    amount: 0,
    description: "",
    invoiceNumber: "",
    dueDate: undefined,
  ...
  // بيانات تسديد دين
  const [paymentData, setPaymentData] = useState<Omit<CreateSupplierPaymentData, 'supplierId'>>({
    amount: 0,
    paymentMethod: "cash",
    referenceNumber: "",
    notes: "",
  ...
  const loadSupplierData = async () => {
    try {
      ...
      const result = await getSupplierWithTransactions(supplierId)
  ...
  const handleAddDebt = async () => {
    try {
      ...
      const transactionData: CreateSupplierTransactionData = {
        supplierId,
        type: 'debt',
        ...debtData
      }

      const result = await addSupplierDebt(transactionData)
      ...
      if (result.success) {
        ...
        await loadSupplierData() // إعادة تحميل البيانات
  ...
  const handleMakePayment = async () => {
    try {
      ...
      const paymentDataWithSupplier: CreateSupplierPaymentData = {
        supplierId,
        ...paymentData
      }

      const result = await makeSupplierPayment(paymentDataWithSupplier)
  ...
  const pendingTransactions = supplier.transactions.filter(t => t.status === 'pending')
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground mt-1">تفاصيل المورد وسجل المعاملات</p>
          </div>
        </div>
          ...
          <Dialog open={isDebtDialogOpen} onOpenChange={setIsDebtDialogOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  إضافة دين جديد للمورد {supplier.name}
                </DialogDescription>
              </DialogHeader>
          ...
          <Dialog open={isPaymentDialogOpen} onOpenChange={setIsPaymentDialogOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  تسجيل دفعة جديدة للمورد {supplier.name}
                </DialogDescription>
              </DialogHeader>
              <div className="space-y-4">
                <div className="space-y-2">
      ...
      <Tabs defaultValue="transactions" className="space-y-4">
        <TabsList>
          <TabsTrigger value="transactions">المعاملات ({supplier.transactions.length})</TabsTrigger>
          <TabsTrigger value="payments">المدفوعات ({supplier.payments.length})</TabsTrigger>
        </TabsList>
